...
var bmsCache = {
	...
	get: function(cell)
	{
		if ( bmsCache.data !== undefined )
		{
			if ( cell in bmsCache.data ) {
				 return bmsCache.data[cell].value;
			 }
		 }
		 return null;
	}	
...
var inverter = {
	...
	/** @brief get the bms values from the inverter */
	getBmsValues: function(replyFunc, includeHidden)
	{
		inverter.sendCmd("bms", function(reply) {
			var params = {};
			try {
				params = JSON.parse(reply);
			} catch(ex) {}

			bmsCache.data = params;
			console.log("bms json");
			console.log(params);
			if (replyFunc) replyFunc(params);
		});
	},
	...
	/** @brief get params/spotvalues from inverter and store them in the cache. */
	refreshBmsValues: function()
	{	
		inverter.getBmsValues(function(params){
			bmsCache.data = params;
		});
	},
