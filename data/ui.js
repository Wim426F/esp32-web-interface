...
var ui = {
	...
	onLoad: function()
	{
		...
		var autoRefresh = setInterval(ui.refresh, 3000);
		ui.bmsInterval = document.getElementById("bmsRefreshInverval").value;
		ui.startInterval(ui.bmsInterval);
	},

	updateInterval: function(){
		// refresh logging interval from input box
		ui.bmsLastInterval = ui.bmsInterval;
		ui.bmsInterval = document.getElementById("bmsRefreshInverval").value;;
		console.log("updateInterval");
		console.log(ui.bmsInterval);
		if (ui.bmsInterval != ui.bmsLastInterval && ui.bmsInterval > 0) {
			clearInterval(ui.bmsIntervalId);
			ui.startInterval(ui.bmsInterval);
		}
	},

	// store in a function so we can call it again
	startInterval: function(_interval) {
		// Store the id of the interval so we can clear it later
		ui.bmsIntervalId = setInterval(function () {ui.refreshBms();}, _interval);
	},
	...
	updateTables: function()
	{
		...
		if ( ! ui.doAutoRefresh ) 
		{ 
			console.log("skipped updateTables()");
			return; 
		}
		...
		for ( var name in paramsCache.data )
		{
			...
			else
			{
				...
				if ( !(param.category in ui.categoryVisibleVal) )
					ui.categoryVisibleVal[param.category] = true;

				if (param.category != lastValCategory)
				{
					var icon = ui.categoryVisibleVal[param.category] ? '-' : '+';
					ui.addRow(tableSpot, [ '<BUTTON onclick="ui.toggleVisibilityVal(\'' + 
						param.category + '\');" style="background: none; border: none; font-weight: bold;">' + icon + ' ' + 
						param.category + '</BUTTON>' ], true);
						lastValCategory = param.category;
				}
				...
				ui.addRow(tableSpot, [ nameWithTooltip, display, unit ], ui.categoryVisibleVal[param.category]);
			}
	...
	/** @brief generates parameter and spotvalue tables */
	updateBmsTable: function()
	{
		var tableBms = document.getElementById("bmsCells");
		var tableTemps = document.getElementById("bmsTemps");
		while (tableBms.rows.length > 1) tableBms.deleteRow(1);
		while (tableTemps.rows.length > 1) tableTemps.deleteRow(1);
		ui.populateCells(tableTemps, 0);
		ui.populateCells(tableBms, 1);
	},
	...
	populateCells: function(table, isCell)
	{
		var module1 = [];
		var module2 = [];
		var module3 = [];
		...
		for (var cell in bmsCache.data)
		{
			var cellNr = bmsCache.data[cell];
			
			if (cellNr.module == 1 && cellNr.iscell == isCell)
			{
				module1[one] = cellNr.value;
				one++;
			}
			if (cellNr.module == 2 && cellNr.iscell == isCell)	
			{
				module2[two] = cellNr.value;
				two++;
			}
			if (cellNr.module == 3 && cellNr.iscell == isCell)	
			{
				module3[three] = cellNr.value;
				three++;
			}
			if (cellNr.module == 4 && cellNr.iscell == isCell)	
			{
				module4[four] = cellNr.value;
				four++;
			}
		}

		for (var n=0; n < Math.max(module1.length, module2.length, module3.length, module4.length); n++)
		{
			...
			if (module1[n] != undefined)	collumn1.innerHTML = module1[n];
			else collumn1.innerHTML = "-";
			if (module2[n] != undefined)	collumn2.innerHTML = module2[n];
			else collumn2.innerHTML = "-";
			if (module3[n] != undefined)	collumn3.innerHTML = module3[n];
			else collumn3.innerHTML = "-";
			if (module4[n] != undefined)	collumn4.innerHTML = module4[n];
			else collumn4.innerHTML = "-";
		}
	},
	...
	toggleVisibilityVal: function(name)
	{
		var rows = document.getElementById("spotValues").rows;
		var found = false;
		ui.categoryVisibleVal[name] = false;
