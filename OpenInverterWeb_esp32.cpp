...
static void handleCommand() {
  ...
  if (!fastUart && fastUartAvailable)
  {
    ...
    if (buffer.startsWith("OK") || buffer.startsWith("fastuart"))
    {
      Serial.begin(USART_BAUDRATE_FAST);
      fastUart = true;
    }
    else
    {
...
void setup(void){
  Serial.setRxBufferSize(50000); // otherwise json won't fit
  Serial.begin(USART_BAUDRATE, SERIAL_8N2);
